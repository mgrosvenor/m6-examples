#!/bin/bash
# dev.sh — start the 03-contact stack for local development.
set -e
...
M6_SOCKET_OVERRIDE=/tmp/m6/render-contact.sock \
    "$EXAMPLES/target/release/render-contact" "$SITE" "$SITE/configs/render-contact.conf" &
RENDER_PID=$!

trap 'kill $HTML_PID $FILE_PID $RENDER_PID $HTTP_PID 2>/dev/null; wait 2>/dev/null' EXIT

# Wait for backend sockets
for i in $(seq 1 30); do
    [ -S /tmp/m6/m6-html.sock ] && \
    [ -S /tmp/m6/m6-file.sock ] && \
    [ -S /tmp/m6/render-contact.sock ] && break
    sleep 0.2
done
