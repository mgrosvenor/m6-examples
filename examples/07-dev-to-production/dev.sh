#!/bin/bash
# dev.sh — start the 07-dev-to-production stack for local development.
set -e
...
sleep 0.3
rm -f /tmp/m6/m6-html*.sock /tmp/m6/m6-file*.sock /tmp/m6/m6-auth*.sock /tmp/m6/render-cms-prod*.sock
...
M6_SOCKET_OVERRIDE=/tmp/m6/render-cms-prod.sock \
    "$EXAMPLES/target/release/render-cms-prod" "$SITE" "$SITE/configs/render-cms.conf" &
RENDER_PID=$!
...
for i in $(seq 1 30); do
    ...
    [ -S /tmp/m6/render-cms-prod.sock ] && break
    sleep 0.2
done
